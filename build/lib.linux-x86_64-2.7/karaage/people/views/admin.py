...
def user_list(request, queryset=Person.objects.select_related()):
    ...
    if 'search' in request.REQUEST:
        ...
        for term in terms.split(' '):
            q = Q(username__icontains=term) | Q(first_name__icontains=term) | Q(last_name__icontains=term) | Q(comment__icontains=term)
            query = query & q
...
def struggling(request):
    ...
    user_accounts = machine_category.useraccount_set.select_related().filter(date_deleted__isnull=True).filter(date_created__lt=days30).filter(last_usage__isnull=True).order_by('-date_created')

    if 'institute' in request.REQUEST:
        institute_id = int(request.GET['institute'])
        user_accounts = user_accounts.filter(institute=institute_id)
