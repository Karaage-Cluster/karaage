...
try:
    ...
    import pipeline.templatetags.pipeline
except ImportError:
    # django-pipeline << 1.4.0
    ...
    def javascript(parser, token):
        return pipeline.templatetags.compressed.compressed_js(parser, token)

    @register.tag
    def stylesheet(parser, token):
        return pipeline.templatetags.compressed.compressed_css(parser, token)
