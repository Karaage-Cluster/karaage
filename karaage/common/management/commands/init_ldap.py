...
class Command(BaseCommand):
    ...
    def _do_server(self, verbose, using):
        ...
        with tldap.transaction.commit_on_success(using=using):
            ...
            base_dn = connection.settings_dict['LDAP_BASE']
            ...
            v, c = ou.objects.using(using).base_dn(base_dn).get_or_create(dn=user_dn)
            if verbose > 0:
                if c:
                    print "Added " + user_dn
                else:
            ...
            v, c = ou.objects.using(using).base_dn(base_dn).get_or_create(dn=group_dn)
            if verbose > 0:
                if c:
