        ...
        if not db.dry_run:
            ...
            for log in orm.logentry.objects.filter(content_type=ct).iterator():
                try:
                    ...
                    # if object_id is not integer, we have to replace it with pk.
                    try:
                        project = orm['projects.project'].objects.get(pid=log.object_id)
        ...
        if not db.dry_run:
            ...
            for log in orm.logentry.objects.filter(content_type=ct).iterator():
                try:
                    ...
                    # if object_id is integer, we have to replace it with pid.
                    project = orm['projects.project'].objects.get(pid=log.object_id)
                    log.object_id = project.pid
                    log.save()
    models = {
        u'common.logentry': {
