...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Account.foreign_id'
        db.add_column('account', 'foreign_id',
                      self.gf('django.db.models.fields.CharField')(max_length=255, null=True, blank=True),
                      keep_default=False)

        # Adding field 'Account.extra_data'
        db.add_column('account', 'extra_data',
                      self.gf('jsonfield.fields.JSONField')(default={}, blank=True, null=True),
                      keep_default=False)

        # Set the default value, this is related to bug
        # http://bugs.mysql.com/bug.php?id=21532
        if not db.dry_run:
            for group in orm.account.objects.iterator():
                if not account.extra_data:
                    account.extra_data = '{}'
                    account.save()

        # Adding unique constraint on 'Account', fields ['foreign_id', 'username', 'machine_category_id']
        db.create_unique('account', ['foreign_id', 'username', 'machine_category_id'])

        # Adding unique constraint on 'Account', fields ['foreign_id', 'machine_category_id']
        db.create_unique('account', ['foreign_id', 'machine_category_id'])

        # Adding unique constraint on 'Account', fields ['username', 'machine_category_id']
        db.create_unique('account', ['username', 'machine_category_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Account', fields ['foreign_id', 'username', 'machine_category_id']
        db.delete_unique('account', ['foreign_id', 'username', 'machine_category_id'])

        # Removing unique constraint on 'Account', fields ['foreign_id', 'machine_category_id']
        db.delete_unique('account', ['foreign_id', 'machine_category_id'])

        # Removing unique constraint on 'Account', fields ['username', 'machine_category_id']
        db.delete_unique('account', ['username', 'machine_category_id'])

        # Deleting field 'Account.foreign_id'
        db.delete_column('account', 'foreign_id')

        # Deleting field 'Account.extra_data'
        db.delete_column('account', 'extra_data')
    ...
    models = {
        ...
        u'machines.account': {
            ...
            'extra_data': ('jsonfield.fields.JSONField', [], {'null': 'True', 'blank': 'True', 'default': '{}'}),
        ...
        u'people.group': {
            ...
            'extra_data': ('json_field.fields.JSONField', [], {'default': "u'null'"}),
        ...
        u'projects.project': {
            ...
            'start_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2014, 1, 30, 0, 0)'})
        }
    }
