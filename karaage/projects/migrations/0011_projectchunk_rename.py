...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(app_label='pbsmoab', model='projectchunk').update(app_label='projects', name='project quota', model='projectquota')

    def backwards(self, orm):
        db.rename_table('project_quota', 'pbsmoab_projectchunk')
        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(app_label='projects', model='projectquota').update(app_label='pbsmoab', name='project chunk', model='projectchunk')
    ...
    models = {
        ...
        u'pbsmoab.projectquota': {
            'Meta': {'object_name': 'ProjectQuota', 'db_table': "'project_quota'"},
            'cap': ('django.db.models.fields.IntegerField', [], {'null': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'machine_category': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['machines.MachineCategory']"}),
    ...
    complete_apps = ['contenttypes', 'pbsmoab']
