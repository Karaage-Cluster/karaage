...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Group.foreign_id'
        db.add_column(u'people_group', 'foreign_id',
        ...
        if not db.dry_run:
            for group in orm.group.objects.iterator():
                if not group.extra_data:
                    group.extra_data = '{}'
                    group.save()

        # Adding unique constraint on 'Group', fields ['name', 'foreign_id']
        db.create_unique(u'people_group', ['name', 'foreign_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'Group', fields ['name', 'foreign_id']
        db.delete_unique(u'people_group', ['name', 'foreign_id'])

        # Deleting field 'Group.foreign_id'
        db.delete_column(u'people_group', 'foreign_id')

        # Deleting field 'Group.extra_data'
        db.delete_column(u'people_group', 'extra_data')
