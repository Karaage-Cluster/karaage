        ...
        if person:
            for job in jobs:
                ...
                if ua:
                    job.user = ua
                    job.save()
    usage_list = CPUJob.objects.filter(Q(project__isnull=True) | Q(isnull=True))
...
def institute_users(request, institute_id, machine_category_id=1):
    ...
    user_total, user_total_jobs = 0, 0
    for u in UserCache.objects.order_by('-cpu_hours').filter(start=start, end=end).filter(project__machine_categories=machine_category).filter(institute=institute).filter(no_jobs__gt=0):
...
def job_list(request):
    ...
    if 'user' in request.REQUEST:
        job_list = job_list.filter(username=request.GET['user'])
    ...
    if 'search' in request.REQUEST:
        ...
        query = Q()
        for term in terms.split(' '):
            q = Q(username__icontains=term) | Q(project__pid__icontains=term)
            query = query & q
